...
1. ajax请求
    ...
    1. 统一错误处理（提示），并允许可配置
    1. 统一成功处理（提示），并允许可配置
    1. 提供4个方法：get put post delete
    1. 超时处理
    1. 打断处理，多个相同异步请求，只保留最后一个
    1. url 统一添加前缀
    1. 添加crsf token
    1. 调用ajax请求的接口，封装成jQuery.ajax风格

1. 数据管理
    1. vuex统一处理数据，单向数据流
    1. 对action进行包装，标准化action数据，遵循 Flux Standard Action
    1. 异步action封装，自动处理error、success提示；自动显示隐藏loading等
    1. 对mutation进行封装，处理异步请求，区分异步不同状态
    1. state 与 localStorage等本地存储数据同步
1. 菜单
    1. 数据来源：前端配置 + 后端请求
    1. 菜单选中状态根据href自动判定，也可以自定义
    1. 菜单过多时，可滚动，并隐藏滚动条，windows下滚动条比较宽（15px左右），比较丑陋
1. 页面头部
    1. 头部标题根据页面状态，自动判断，也可以自定义
    1. 面包屑导航根据页面状态自动判定，也可以自定义
    1. 页面头部，当前页面可以决定显示隐藏，通过一个函数调用或者一个配置变量进行设置

1. 提供全局事件总线（发布订阅），解决跨组件事件通信
1. permission组件封装，根据permission code，决定按钮是否显示，权限控制相关功能会用到。
1. 路由
    1. 路由拆分到各个模块中，解决一个路由文件过大，多人协作版本冲突问题
    1. 根据不同模块路由，构建不同模块，开发过程中，可以只构建自己关心的模块，可以提高构建速度
    1. 通过route-loader 简化按需加载页面路由写法
1. 全局公共方法提供，高频使用的方法，绑定到Vue实例上，直接调用即可，不用引用
1. 主题支持
1. mock功能，提供ajax数据，方便前端开发调试
1. 浏览器localStorage、sessionStorage封装，支持String 与 JSON互转
1. font-icon 封装
1. 其他组价封装。。。
